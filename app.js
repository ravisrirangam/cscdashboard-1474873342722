...
app.post('/api/searchuser', function(request, response){
 ...
 console.log("POST method invoked..Verifying the user profile...");
...
response.setHeader('Content-Type', 'application/json');
    ...
    if(result.docs.length == 0){
      ...
      response.write('{\"Userid\":\"null\", \"PWCheck\":\"null\", \"Role\":\"null\"}');
  ...
  return;
      ...
      var userRole = result.docs[i].role;
      ...
      if(!(result.docs[i].password == request.body.password)){
        ...
        response.write('{\"Userid\":\"'+request.body.userid+'\", \"PWCheck\":\"False\", \"Role\":\"'+userRole+'\"}');
        ...
        response.write('{\"Userid\":\"'+request.body.userid+'\", \"PWCheck\":\"True\", \"Role\":\"'+userRole+'\"}');
...
//         localuserdb.insert({ "userid":userid, "password":password}, function(err, body) {
        ...
        userdb.insert({ "userid":userid, "password":password, "role":role}, function(err, body) {
